...
texture_names = [i for i in range(0, 24)]
...
def load_textures():
    ...
    # Load images from file system  frame_00_delay-0.06s.png
    images = [pygame.image.load(os.path.join('', f'frame_{i}_delay-0.06s.png')) for i in range(0, 24)]
...
def timer_func(value):
    ...
    glutTimerFunc(50, timer_func, 0)  # Schedule the next redraw in 100 milliseconds
